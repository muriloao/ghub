...
import 'login_with_credentials_test.mocks.dart';
...
void main() {
  ...
  group('LoginWithCredentials', () {
    ...
    test('should return AuthResult when login is successful', () async {
      ...
      verify(mockAuthRepository.loginWithCredentials(
        email: validEmail,
        password: validPassword,
      ));
    ...
    test('should return ValidationFailure when email is empty', () async {
      ...
      // assert
      expect(result, const Left(ValidationFailure(message: 'Email e senha são obrigatórios')));
    ...
    test('should return ValidationFailure when email is invalid', () async {
      ...
      // assert
      expect(result, const Left(ValidationFailure(message: 'Email inválido')));
      verifyZeroInteractions(mockAuthRepository);
    });
    ...
    test('should return ValidationFailure when password is too short', () async {
      ...
      // assert
      expect(result, const Left(ValidationFailure(message: 'Senha deve ter pelo menos 6 caracteres')));
      verifyZeroInteractions(mockAuthRepository);
    });
